...
from ..helpers.field_helpers import deferred_list_field, fk_field, templated_field
...
if TYPE_CHECKING:
    ...
    from .image_builder_model import ImageBuilderModel

@dataclass(config=CSIS_MODEL_CONFIG)
class ImageImageBuilderSubconfig(NameTyped, ParentPropertyHoldingProtocol):
    ...
    # def __init__(self, runtime: str, image_identifier: str | None = None, machine_type: str | None = None, **kwargs):
    ...
    def __post_init__(self):
        ...
        if self.image_builder in OOPS_DEFAULTS:
            # No image_builder given: fall back to the registered default so the
            # subconfig gets a real identity (name/type derive from it below).
            dflt = Registry().get_default_for(VCT.IMAGE_BUILDER_MODEL)
            if dflt:
                self.image_builder = dflt
        self.type_ = self.image_builder # There maybe needs to be a type specified
        self.name = self.image_builder if self.name == DEFAULT else self.name
        if not self.tags:
            self.tags = {}
        if not self.owners:
            self.owners = [SELF]
    ...
    def get_image_output_name(self) -> str:
        ...
        ret = f"{self._image.get_display_name()}-{self.image_builder}" if self._image and self._image.name else f"{self.model_id}-{self.image_builder}"
        return ret
    ...
    def get_description(self) -> str | None:
        return f"Runtime-specific configuration for runtime {self.image_builder} for image {self.model_id}"
    def get_aliases(self) -> set[str]:
        return set()
    def get_classification(self) -> VCT:
        return self.csis_classifier()
    def get_runtime(self) -> str:
        return self._get_image_builder_model().get_runtime_provider()
    def get_image_builder(self) -> str:
        return self._get_image_builder_model().get_name()
    def _get_image_builder_model(self) -> ImageBuilderModel:
        from .image_builder_model import ImageBuilderModel
        ret = self._reg.get_instance_by_name_or_alias(VCT.IMAGE_BUILDER_MODEL, self.image_builder) if self.image_builder else None
        if ret is None:
            errstr = f"Image builder {self.image_builder} specified for runtime configuration {self.name} not found in registry."
            log.error(errstr)
            raise ValueError(errstr)
        assert isinstance(ret, ImageBuilderModel), (
            f"Registry entry for image builder {self.image_builder!r} is "
            f"{type(ret).__name__}, expected ImageBuilderModel")
        return ret
...
@dataclass(config=CSIS_MODEL_CONFIG)
class Image(RootItem,SelfInjectedNameProtocol):
    ...
    # Convergent bakes (stage 9): how this image follows its parent series.
    # pinned (N17, default): bakes FROM the pinned parent build until an explicit
    # `upgrade image`; follow: a newer parent head re-bakes this image from it and
    # moves the pin (recorded as op: follow).
    parent_policy: str = "pinned"
    # Image retention (stage 10.3): {keep: N} = the newest N builds of this series
    # on a runtime survive; older ones are disposed by the closing `retention`
    # lifecycle (never a build an instance is pinned to or launched from -- those
    # are retention debt). None = the runtime's retention_keep, else keep all.
    retention: dict[str, Any] | None = None
    # Declared release intent (stage 14, ledger 71): {model: <m>} -- a build of
    # this image whose post-bake tests PASSED in a run is released for the
    # model by the `release` lifecycle of that same run (between the
    # instance-image lifecycle that verified it and the closing `retention`,
    # which keeps released builds). Without it releases are the operator's
    # explicit `release` command -- which on an ephemeral runtime has no
    # moment to happen: retention disposes the build first (found live).
    ...
    modifications: list[dict[str,Any]] = deferred_list_field(builder_vct=VCT.MOD_BUILDER_MODEL,
                                                             item_vct=VCT.MOD_BUILDER_ITEM_MODEL)
    tags: dict[str, str] = field(default_factory=dict)
    # V2 (Q4, §3F2): an instance image belongs to exactly ONE group. The
    # group name is system-unique, so it alone identifies the backing
    # identity-manager type; every instance of this image shares the group.
    group: str | None = None
    # EXPLORE image tests: declarative in-bake assertions (files,
    # packages, commands, services, users) run as the LAST provisioner; a
    # failing assertion fails the bake.
    tests: dict[str, Any] = field(default_factory=dict)
    default_groups: list[str] = field(default_factory=list)
    runtimes: list[ImageImageBuilderSubconfig] = field(default_factory=list)
    ...
    def __post_init__(self):
        ...
        self._runtime_map: dict[str, ImageImageBuilderSubconfig] = {}
        for r in self.runtimes:
            r.model_id = self.global_id
            # Subconfigs are image_builder-keyed; the runtime map keys by the
            # image builder's runtime provider so per-runtime lookups still work.
            self._runtime_map[r.get_runtime()] = r
            r._image = self
        ...
        if self.description is None:
            ...
            # elif self.source_image:
            self.description = f"Image {self.name} from source image {self.source_image}"
    ...
    def finalize(self) -> None:
        ...
        #     __osb = reg.get_instance_by_name_or_alias( VCT.OS_BUILDER_MODEL, self.os)
        #     if not __osb:
    ...
    def image_builder(self) -> ImageBuilderBase|None:
        ...
        if not self._image_builder:
            if self.source_image:
                ...
                reg = Registry()
                inst = reg.get_all_instances_by_classification(VCT.IMAGE_MODEL)
            ...
            if self._osbuilder:
                ...
                _ctx = GlobalTypeContext()
                ctx =  _ctx.image_builders
    ...
    # This property is a self-mutator.  It sets a value if not already set
    # @property
    # def final_name(self) -> str:
    #     if self._final_name is None:
    #         # TODO Setup final name based on OS builder/runtime/etc
    #         self._final_name = self.name
    #     return self._final_name
    ...
    def get_image_runtime_subconfig_for_runtime(self, runtime: str) -> ImageImageBuilderSubconfig | None:
