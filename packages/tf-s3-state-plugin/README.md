...
| `Object TofuS3StateBuilderModel(...) has no unique 'id' or 'name' field for registry key.` at load, after a long model dump | two entries share a name after normalisation (`S3-East2` and `s3-east2` are one name); the message is misleading | every `cfg/state-backends*.yml` | rename one |
