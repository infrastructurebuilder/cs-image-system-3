# Build configuration for Packer EBS Ansible Builder generated by PackerEbsImageBuilder (pckr-ebs-ans)
build {
  name = "pckr-ebs-ans-block-000"
  sources = [
"source.amazon-ebs.basic-rhel-8",
  "source.amazon-ebs.basic-rhel-9",
  "source.amazon-ebs.my-deb-11"
]
  # OS update for base image basic-rhel-8 (policy=security, packages=['openssl'], exclude=['kernel*'], pin={}, rhel)
  provisioner "shell" {
    only   = ["amazon-ebs.basic-rhel-8"]
    inline = ["# subscription-managed systems get their repos enabled; RHUI/PAYG images skip", "if sudo subscription-manager identity >/dev/null 2>&1; then sudo subscription-manager refresh; sudo subscription-manager repos --disable='*'; sudo subscription-manager repos --enable='rhel-8-for-x86_64-baseos-rpms' --enable='rhel-8-for-x86_64-appstream-rpms' --enable='codeready-builder-for-rhel-8-x86_64-rpms'; else echo 'not subscription-registered (RHUI image): using vendor repos as-is'; fi", "sudo dnf clean all", "sudo dnf -y update --security --exclude=kernel* || { sudo dnf clean all; sudo dnf -y update --security --exclude=kernel*; }", "sudo dnf -y update --exclude=kernel* openssl || { sudo dnf clean all; sudo dnf -y update --exclude=kernel* openssl; }", "sudo mkdir -p /var/lib/csis", "( rpm -qa --qf '%%{NAME}-%%{VERSION}-%%{RELEASE}.%%{ARCH}\n' 2>/dev/null || dpkg-query -W -f='$${Package}=$${Version}\n' ) | sort | sudo tee /var/lib/csis/packages.txt >/dev/null"]
  }
  # admin user csisadmin (1 public key(s))
  provisioner "shell" {
    only   = ["amazon-ebs.basic-rhel-8"]
    inline = [
      "# mandatory local admin user 'csisadmin' (V2_PLAN Q2/N9): public keys only",
      "id -u csisadmin >/dev/null 2>&1 || sudo useradd -m -s /bin/bash csisadmin",
      "echo 'csisadmin ALL=(ALL) NOPASSWD:ALL' | sudo tee /etc/sudoers.d/90-csisadmin >/dev/null && sudo chmod 0440 /etc/sudoers.d/90-csisadmin",
      "sudo mkdir -p /home/csisadmin/.ssh && sudo chmod 0700 /home/csisadmin/.ssh",
      "sudo rm -f /home/csisadmin/.ssh/authorized_keys",
      "echo 'ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIPlaceholderKeyReplaceMeReplaceMeReplaceMe csis-admin-placeholder' | sudo tee -a /home/csisadmin/.ssh/authorized_keys >/dev/null",
      "sudo touch /home/csisadmin/.ssh/authorized_keys && sudo chmod 0600 /home/csisadmin/.ssh/authorized_keys",
      "sudo chown -R csisadmin:csisadmin /home/csisadmin/.ssh",
    ]
  }
  # identity type 'okta' prerequisites, dormant
  provisioner "shell" {
    only   = ["amazon-ebs.basic-rhel-8"]
    inline = [
      "# identity type 'okta' prerequisites (oktagroups): OPA agent, dormant",
      "sudo rpm --import https://dist.scaleft.com/GPG-KEY-OktaPAM-2023",
      "printf '[oktapam-stable]\\nname=Okta PAM Stable - $basearch\\nbaseurl=https://dist.scaleft.com/repos/rpm/stable/rhel/$releasever/$basearch\\ngpgcheck=1\\nrepo_gpgcheck=1\\nenabled=1\\ngpgkey=https://dist.scaleft.com/GPG-KEY-OktaPAM-2023\\n' | sudo tee /etc/yum.repos.d/oktapam-stable.repo",
      "sudo yum install -y scaleft-server-tools",
      "sudo systemctl disable --now sftd || true",
      "sudo rm -f /var/lib/sftd/enrollment.token",
    ]
  }
  # storage type 'ebs' prerequisites
  provisioner "shell" {
    only   = ["amazon-ebs.basic-rhel-8"]
    inline = [
      "# storage type 'ebs' declared: no prerequisites to bake (aws-ebs)",
    ]
  }
  # storage type 'efs' prerequisites
  provisioner "shell" {
    only   = ["amazon-ebs.basic-rhel-8"]
    inline = [
      "# storage type 'efs' prerequisites (aws-efs): NFS/EFS mount tooling",
      "sudo yum install -y amazon-efs-utils || sudo yum install -y nfs-utils",
    ]
  }
  # storage type 's3' prerequisites
  provisioner "shell" {
    only   = ["amazon-ebs.basic-rhel-8"]
    inline = [
      "# storage type 's3' prerequisites (aws-s3): AWS CLI for object access",
      "command -v aws >/dev/null 2>&1 || { command -v unzip >/dev/null 2>&1 || sudo yum install -y unzip; curl -fsSL https://awscli.amazonaws.com/awscli-exe-linux-x86_64.zip -o /tmp/awscliv2.zip && cd /tmp && unzip -q awscliv2.zip && sudo ./aws/install; }",
    ]
  }
  # debug session mechanism (ssm)
  provisioner "shell" {
    only   = ["amazon-ebs.basic-rhel-8"]
    inline = [
      "# session mechanism 'ssm' (aws-east2-runtime): SSM agent for debug sessions",
      "sudo yum install -y https://s3.us-east-2.amazonaws.com/amazon-ssm-us-east-2/latest/linux_amd64/amazon-ssm-agent.rpm || sudo yum install -y amazon-ssm-agent",
      "sudo systemctl enable amazon-ssm-agent",
    ]
  }
  # in-bake verification for base image basic-rhel-8: 13 assertion(s)
  provisioner "shell" {
    only   = ["amazon-ebs.basic-rhel-8"]
    inline = [
      "set -e",
      "# verify: admin user csisadmin",
      "id -u csisadmin >/dev/null 2>&1",
      "sudo test -f /home/csisadmin/.ssh/authorized_keys",
      "sudo grep -q -- AAAAC3NzaC1lZDI1NTE5AAAAIPlaceholderKeyReplaceMeReplaceMeReplaceMe /home/csisadmin/.ssh/authorized_keys",
      "# verify: OPA agent present and dormant",
      "command -v sftd >/dev/null 2>&1 || test -x /usr/sbin/sftd || test -x /usr/bin/sftd",
      "! systemctl is-enabled sftd >/dev/null 2>&1",
      "test ! -e /var/lib/sftd/enrollment.token",
      "# verify: EFS/NFS mount tooling present",
      "command -v mount.efs >/dev/null 2>&1 || command -v mount.nfs >/dev/null 2>&1 || command -v mount.nfs4 >/dev/null 2>&1",
      "# verify: AWS CLI present",
      "command -v aws >/dev/null 2>&1",
      "# verify: SSM agent baked",
      "command -v amazon-ssm-agent >/dev/null 2>&1 || test -x /usr/bin/amazon-ssm-agent || snap list amazon-ssm-agent >/dev/null 2>&1",
      "systemctl is-enabled amazon-ssm-agent >/dev/null 2>&1 || systemctl is-enabled snap.amazon-ssm-agent.amazon-ssm-agent.service >/dev/null 2>&1",
      "test -e /etc/yum.repos.d/oktapam-stable.repo",
      "grep -q -- dist.scaleft.com /etc/yum.repos.d/oktapam-stable.repo",
      "rpm -q nfs-utils >/dev/null 2>&1 || { command -v dpkg >/dev/null 2>&1 && dpkg -s nfs-utils >/dev/null 2>&1; }",
    ]
  }
  # OS update for base image basic-rhel-9 (policy=security, packages=[], exclude=['kernel*'], pin={}, rhel)
  provisioner "shell" {
    only   = ["amazon-ebs.basic-rhel-9"]
    inline = ["# subscription-managed systems get their repos enabled; RHUI/PAYG images skip", "if sudo subscription-manager identity >/dev/null 2>&1; then sudo subscription-manager refresh; sudo subscription-manager repos --disable='*'; sudo subscription-manager repos --enable='rhel-9-for-x86_64-baseos-rpms' --enable='rhel-9-for-x86_64-appstream-rpms' --enable='codeready-builder-for-rhel-9-x86_64-rpms'; else echo 'not subscription-registered (RHUI image): using vendor repos as-is'; fi", "sudo dnf clean all", "sudo dnf -y update --security --exclude=kernel* || { sudo dnf clean all; sudo dnf -y update --security --exclude=kernel*; }", "sudo mkdir -p /var/lib/csis", "( rpm -qa --qf '%%{NAME}-%%{VERSION}-%%{RELEASE}.%%{ARCH}\n' 2>/dev/null || dpkg-query -W -f='$${Package}=$${Version}\n' ) | sort | sudo tee /var/lib/csis/packages.txt >/dev/null"]
  }
  # admin user csisadmin (1 public key(s))
  provisioner "shell" {
    only   = ["amazon-ebs.basic-rhel-9"]
    inline = [
      "# mandatory local admin user 'csisadmin' (V2_PLAN Q2/N9): public keys only",
      "id -u csisadmin >/dev/null 2>&1 || sudo useradd -m -s /bin/bash csisadmin",
      "echo 'csisadmin ALL=(ALL) NOPASSWD:ALL' | sudo tee /etc/sudoers.d/90-csisadmin >/dev/null && sudo chmod 0440 /etc/sudoers.d/90-csisadmin",
      "sudo mkdir -p /home/csisadmin/.ssh && sudo chmod 0700 /home/csisadmin/.ssh",
      "sudo rm -f /home/csisadmin/.ssh/authorized_keys",
      "echo 'ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIPlaceholderKeyReplaceMeReplaceMeReplaceMe csis-admin-placeholder' | sudo tee -a /home/csisadmin/.ssh/authorized_keys >/dev/null",
      "sudo touch /home/csisadmin/.ssh/authorized_keys && sudo chmod 0600 /home/csisadmin/.ssh/authorized_keys",
      "sudo chown -R csisadmin:csisadmin /home/csisadmin/.ssh",
    ]
  }
  # identity type 'okta' prerequisites, dormant
  provisioner "shell" {
    only   = ["amazon-ebs.basic-rhel-9"]
    inline = [
      "# identity type 'okta' prerequisites (oktagroups): OPA agent, dormant",
      "sudo rpm --import https://dist.scaleft.com/GPG-KEY-OktaPAM-2023",
      "printf '[oktapam-stable]\\nname=Okta PAM Stable - $basearch\\nbaseurl=https://dist.scaleft.com/repos/rpm/stable/rhel/$releasever/$basearch\\ngpgcheck=1\\nrepo_gpgcheck=1\\nenabled=1\\ngpgkey=https://dist.scaleft.com/GPG-KEY-OktaPAM-2023\\n' | sudo tee /etc/yum.repos.d/oktapam-stable.repo",
      "sudo yum install -y scaleft-server-tools",
      "sudo systemctl disable --now sftd || true",
      "sudo rm -f /var/lib/sftd/enrollment.token",
    ]
  }
  # storage type 'ebs' prerequisites
  provisioner "shell" {
    only   = ["amazon-ebs.basic-rhel-9"]
    inline = [
      "# storage type 'ebs' declared: no prerequisites to bake (aws-ebs)",
    ]
  }
  # storage type 'efs' prerequisites
  provisioner "shell" {
    only   = ["amazon-ebs.basic-rhel-9"]
    inline = [
      "# storage type 'efs' prerequisites (aws-efs): NFS/EFS mount tooling",
      "sudo yum install -y amazon-efs-utils || sudo yum install -y nfs-utils",
    ]
  }
  # storage type 's3' prerequisites
  provisioner "shell" {
    only   = ["amazon-ebs.basic-rhel-9"]
    inline = [
      "# storage type 's3' prerequisites (aws-s3): AWS CLI for object access",
      "command -v aws >/dev/null 2>&1 || { command -v unzip >/dev/null 2>&1 || sudo yum install -y unzip; curl -fsSL https://awscli.amazonaws.com/awscli-exe-linux-x86_64.zip -o /tmp/awscliv2.zip && cd /tmp && unzip -q awscliv2.zip && sudo ./aws/install; }",
    ]
  }
  # debug session mechanism (ssm)
  provisioner "shell" {
    only   = ["amazon-ebs.basic-rhel-9"]
    inline = [
      "# session mechanism 'ssm' (aws-east2-runtime): SSM agent for debug sessions",
      "sudo yum install -y https://s3.us-east-2.amazonaws.com/amazon-ssm-us-east-2/latest/linux_amd64/amazon-ssm-agent.rpm || sudo yum install -y amazon-ssm-agent",
      "sudo systemctl enable amazon-ssm-agent",
    ]
  }
  # in-bake verification for base image basic-rhel-9: 10 assertion(s)
  provisioner "shell" {
    only   = ["amazon-ebs.basic-rhel-9"]
    inline = [
      "set -e",
      "# verify: admin user csisadmin",
      "id -u csisadmin >/dev/null 2>&1",
      "sudo test -f /home/csisadmin/.ssh/authorized_keys",
      "sudo grep -q -- AAAAC3NzaC1lZDI1NTE5AAAAIPlaceholderKeyReplaceMeReplaceMeReplaceMe /home/csisadmin/.ssh/authorized_keys",
      "# verify: OPA agent present and dormant",
      "command -v sftd >/dev/null 2>&1 || test -x /usr/sbin/sftd || test -x /usr/bin/sftd",
      "! systemctl is-enabled sftd >/dev/null 2>&1",
      "test ! -e /var/lib/sftd/enrollment.token",
      "# verify: EFS/NFS mount tooling present",
      "command -v mount.efs >/dev/null 2>&1 || command -v mount.nfs >/dev/null 2>&1 || command -v mount.nfs4 >/dev/null 2>&1",
      "# verify: AWS CLI present",
      "command -v aws >/dev/null 2>&1",
      "# verify: SSM agent baked",
      "command -v amazon-ssm-agent >/dev/null 2>&1 || test -x /usr/bin/amazon-ssm-agent || snap list amazon-ssm-agent >/dev/null 2>&1",
      "systemctl is-enabled amazon-ssm-agent >/dev/null 2>&1 || systemctl is-enabled snap.amazon-ssm-agent.amazon-ssm-agent.service >/dev/null 2>&1",
    ]
  }
  # admin user csisadmin (1 public key(s))
  provisioner "shell" {
    only   = ["amazon-ebs.my-deb-11"]
    inline = [
      "# mandatory local admin user 'csisadmin' (V2_PLAN Q2/N9): public keys only",
      "id -u csisadmin >/dev/null 2>&1 || sudo useradd -m -s /bin/bash csisadmin",
      "echo 'csisadmin ALL=(ALL) NOPASSWD:ALL' | sudo tee /etc/sudoers.d/90-csisadmin >/dev/null && sudo chmod 0440 /etc/sudoers.d/90-csisadmin",
      "sudo mkdir -p /home/csisadmin/.ssh && sudo chmod 0700 /home/csisadmin/.ssh",
      "sudo rm -f /home/csisadmin/.ssh/authorized_keys",
      "echo 'ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIPlaceholderKeyReplaceMeReplaceMeReplaceMe csis-admin-placeholder' | sudo tee -a /home/csisadmin/.ssh/authorized_keys >/dev/null",
      "sudo touch /home/csisadmin/.ssh/authorized_keys && sudo chmod 0600 /home/csisadmin/.ssh/authorized_keys",
      "sudo chown -R csisadmin:csisadmin /home/csisadmin/.ssh",
    ]
  }
  # identity type 'okta' prerequisites, dormant
  provisioner "shell" {
    only   = ["amazon-ebs.my-deb-11"]
    inline = [
      "# identity type 'okta' prerequisites (oktagroups): OPA agent, dormant",
      "command -v curl >/dev/null 2>&1 && command -v gpg >/dev/null 2>&1 || { sudo apt-get -o DPkg::Lock::Timeout=600 update -y; sudo apt-get -o DPkg::Lock::Timeout=600 install -y curl gnupg; }",
      "curl -fsSL https://dist.scaleft.com/GPG-KEY-OktaPAM-2023 | sudo gpg --dearmor -o /usr/share/keyrings/oktapam-2023-archive-keyring.gpg",
      "echo 'deb [signed-by=/usr/share/keyrings/oktapam-2023-archive-keyring.gpg] https://dist.scaleft.com/repos/deb focal okta' | sudo tee /etc/apt/sources.list.d/oktapam-stable.list",
      "sudo apt-get -o DPkg::Lock::Timeout=600 update -y",
      "sudo apt-get -o DPkg::Lock::Timeout=600 install -y scaleft-server-tools",
      "sudo systemctl disable --now sftd || true",
      "sudo rm -f /var/lib/sftd/enrollment.token",
    ]
  }
  # storage type 'ebs' prerequisites
  provisioner "shell" {
    only   = ["amazon-ebs.my-deb-11"]
    inline = [
      "# storage type 'ebs' declared: no prerequisites to bake (aws-ebs)",
    ]
  }
  # debug session mechanism (ssm)
  provisioner "shell" {
    only   = ["amazon-ebs.my-deb-11"]
    inline = [
      "# session mechanism 'ssm' (aws-east2-runtime): SSM agent for debug sessions",
      "command -v amazon-ssm-agent >/dev/null 2>&1 || snap list amazon-ssm-agent >/dev/null 2>&1 || (sudo curl -fsSL https://s3.us-east-2.amazonaws.com/amazon-ssm-us-east-2/latest/debian_amd64/amazon-ssm-agent.deb -o /tmp/ssm-agent-bake.deb && sudo dpkg -i /tmp/ssm-agent-bake.deb)",
      "sudo systemctl enable amazon-ssm-agent",
    ]
  }
  # in-bake verification for base image my-deb-11: 8 assertion(s)
  provisioner "shell" {
    only   = ["amazon-ebs.my-deb-11"]
    inline = [
      "set -e",
      "# verify: admin user csisadmin",
      "id -u csisadmin >/dev/null 2>&1",
      "sudo test -f /home/csisadmin/.ssh/authorized_keys",
      "sudo grep -q -- AAAAC3NzaC1lZDI1NTE5AAAAIPlaceholderKeyReplaceMeReplaceMeReplaceMe /home/csisadmin/.ssh/authorized_keys",
      "# verify: OPA agent present and dormant",
      "command -v sftd >/dev/null 2>&1 || test -x /usr/sbin/sftd || test -x /usr/bin/sftd",
      "! systemctl is-enabled sftd >/dev/null 2>&1",
      "test ! -e /var/lib/sftd/enrollment.token",
      "# verify: SSM agent baked",
      "command -v amazon-ssm-agent >/dev/null 2>&1 || test -x /usr/bin/amazon-ssm-agent || snap list amazon-ssm-agent >/dev/null 2>&1",
      "systemctl is-enabled amazon-ssm-agent >/dev/null 2>&1 || systemctl is-enabled snap.amazon-ssm-agent.amazon-ssm-agent.service >/dev/null 2>&1",
    ]
  }
  post-processor "manifest" {
    output     = "manifest.json"
    strip_path = true
  }
}