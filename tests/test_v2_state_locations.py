...
NEW = "s3://new-bucket/statefiles/new/ws.tfstate"
...
def test_every_root_resolves_a_location_the_run_records_and_the_emission_carries(tmp_path, monkeypatch):
    ...
    assert rec["aws-ebs"] == {"backend": "s3-east1", "type": "s3", "location": f"{EAST1}/aws_ebs.tfstate",
                              "bucket": "my-east1-tfstate-bucket", "key": "statefiles/csia/aws_ebs.tfstate",
...
def test_a_migration_run_emits_the_operation_for_that_root_alone(tmp_path, monkeypatch):
    ...
    for needle in ("state-migration begin --workspace aws-efs --tofu",
                   ...
                   f"--location {EAST2}/aws_efs.tfstate",
...
def migration(tmp_path):
    ...
    ms.record_state_locations({"ws": {"backend": "s3-east1", "type": "s3",
                                      "location": "s3://old-bucket/statefiles/old/ws.tfstate",
                                      "bucket": "old-bucket", "key": "statefiles/old/ws.tfstate",
                                      "region": "us-east-1", "encrypt": True, "use_lockfile": True}}, "r0")
...
def test_begin_backs_the_old_state_up_and_finish_records_the_move(migration):
    ...
    assert begin(migration.ctx, "ws", migration.tofu, Path("ws.tfbackend.hcl"), "r1", migration.root, new_location=NEW) == 0
    ...
    assert move["from"]["location"] == "s3://old-bucket/statefiles/old/ws.tfstate" and move["to"]["location"] == NEW
...
def test_begin_refuses_a_new_location_that_holds_someone_elses_state(migration, caplog):
    ...
    with caplog.at_level(logging.ERROR):
        assert begin(migration.ctx, "ws", migration.tofu, Path("ws.tfbackend.hcl"), "r1", migration.root, new_location=NEW) == 1
...
def test_begin_recognises_a_move_that_already_happened(migration):
    ...
    assert begin(migration.ctx, "ws", migration.tofu, Path("ws.tfbackend.hcl"), "r2", migration.root, new_location=NEW) == 0
